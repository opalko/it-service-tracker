...
# --- AUTOCOMPLETE FIELD ---
def autocomplete_field(label, field_name, key_prefix=""):
    ...
    key_base = f"{key_prefix}_{label.lower().replace(' ', '_')}"
...
with st.form("new_call_form", clear_on_submit=True):
    ...
    with col1:
        ...
        client = autocomplete_field("Client", "client", key_prefix="new")
        department = autocomplete_field("Department", "department", key_prefix="new")
        call_type = autocomplete_field("Call Type", "call_type", key_prefix="new")
        status = autocomplete_field("Status", "status", key_prefix="new")
